angular.module('laReta.controllers', [])
    ...
    .controller('AppCtrl', function($scope, $localStorage) {
        ...
        $scope.user = $localStorage.get('user', {role: 'ROLE_USER'}, true);
    ...
    .controller('ProfileSelfCtrl', function($scope, apiHandler, $state, $rootScope,
        ...
        $cordovaDevice, dateUtility, $stateParams) {
        // Initialize user
        $scope.user = {};
        $scope.imageData = {};
...
/*        $scope.toggleDateBox = function () {
    ...
    .controller('CanchaCtrl', function($rootScope, $scope, $location,
        ...
        promise.then(function (result) {
            if (result.error != 0) {
                ...
                $scope.cancha.hora_inicio = new Date("2017-08-31 " + result.data.hora_inicio);
                $scope.cancha.hora_fin = new Date("2017-08-31 " + result.data.hora_fin);
    ...
    .controller('ReservacionesCtrl', function($rootScope, $scope, $stateParams, apiHandler, $state, $ionicModal,
        ...
        $scope.goToFecha = function(obj){
            ...
            promise.then(function (result) {
                ...
                if (result.error != 0) {
                    ...
                    var reservaciones = result.data;

                    for(var cont=0; reservaciones.length > cont; cont++){
                        reservaciones[cont].hora_inicio = new Date( reservaciones[cont].fecha + " " + reservaciones[cont].hora_inicio );
                        reservaciones[cont].hora_fin = new Date( reservaciones[cont].fecha + " " + reservaciones[cont].hora_fin );
                    }
        ...
        $scope.showReservacion = function(reservacion){
            ...
            $localStorage.set("new_event", { } ,true);
            $localStorage.set("reservacion_user", reservacion ,true);
            $state.go('app.cancha', {'canchaId': reservacion.cancha.id});
        }

    })
    .controller('UsuariosAdminCanchasCtrl', function($rootScope, $scope, $location,
                                        $stateParams, apiHandler, facebookHandler, $state, $ionicHistory, uiGmapGoogleMapApi,
                                        $ionicModal, $ionicPopup, $cordovaDatePicker, $cordovaDevice, dateUtility,
                                        $localStorage, $cordovaGeolocation, $ionicScrollDelegate, PaypalService, $cordovaCamera) {
        ...
        $scope.getUsuarios = function() {
            var promise = null;
            promise = apiHandler.listUsersHasCanchas({});
        ...
        $ionicModal.fromTemplateUrl('usuario-admin-canchas.html', {
            scope: $scope
        }).then(function (modalView) {
            $scope.perfilModal = modalView;
        });

        $scope.permiso = function(role){
            var data = {role: role, has_canchas: $scope.usuario.has_canchas, usuarioId: $scope.usuario.id};
            if(role == 'ROLE_USER'){
                data.has_canchas = false;
            }
            var promise = null;
            promise = apiHandler.updateRole(data);
        ...
        // Accion para mostrar el formCalendarioCancha
        $scope.showPerfil = function (usuario) {
